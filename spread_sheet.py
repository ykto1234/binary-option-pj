...
def copy_worksheet(workbook, src_title: str, dist_title: str):
    # ワークシート追加
    worksheet = workbook.worksheet(src_title)
    workbook.duplicate_sheet(
        source_sheet_id=worksheet.id, new_sheet_name=dist_title)
    return
...
def import_gspread(auth_keyfile_path, csv_file_path, gspread_folderid, book_name, sheet_title):
    ...
    if book_name not in title_list:
        ...
        # スプレッドシートIDを取得
        sheet_id = [file['id']
                    for file in file_list if file['title'] == book_name]
        sheet_id = sheet_id[0]
        # ブックを開く
        workbook = gc.open_by_key(sheet_id)
        # 存在するワークシートの情報を全て取得
